...
if(input[input.sourceType+'current_payload_option']==="raw"){
    // result: JSON
    result = Resources["ContentLoaderFunctions"].PutJSON(params);
    //logger.warn(input.sourceType+": "+"Put Basic auth Response : "+JSON.stringify(result));
    delete result.headers;
} else if(input[input.sourceType+'current_payload_option']==="x-www-form-urlencoded"){
    result = Resources["ContentLoaderFunctions"].PutText(params);
    if(Things["GenericIEMasterConfig"].isEnableScriptLogs === true) {
    	logger.warn(input.sourceType+": "+"Put text basic auth Response : "+JSON.stringify(result));
    }
}