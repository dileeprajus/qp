...
public class RFQResponseModel extends GenericObjectService{
//private static final Logger LOGGER = LogR.getLogger(RFQResponseModel.class.getName());
//private static final String CLASSNAME = RFQResponseModel.class.getName();
  ...
  /**
    * This method is used delete RFQ Response of given oid,
    * @param oid String 
    * @exception Exception
    ...
    */
    ...
    //if (LOGGER.isDebugEnabled())
      ...
      try{
        RFQResponse rfqResponse = (RFQResponse) LCSQuery.findObjectById(oid);
        RFQLogic rfqLogic = new RFQLogic();
        rfqLogic.deleteRFQResponse(rfqResponse);
        responseObject=util.getDeleteResponseObject("RFQ Response",oid,responseObject);
    ...
    /**
      * This method is used to call Associations objects of the Flex type SourcingConfiguration.
      * @param  
      * @param 
      * @param 
      * @exception Exception
      * @return 
      */
    private void sourcingConfigurationAssociations( ) throws Exception{
    }
  ...
  public FlexTypeClassificationTreeLoader getFlexTypeHierarchy() throws Exception{
        return new FlexTypeClassificationTreeLoader("com.lcs.wc.sourcing.RFQResponse"); 
  } 
            ...
            if(configObject!=null){
                ...
                if((typeId!=null) && !("".equalsIgnoreCase(typeId))){
                    ...
                    while (totalAttrsListItr.hasNext()) {
                        ...
                        if (attribute.getAttScope().equals("RESPONSE_HEADER_SCOPE")) {
  ...
  public String searchByName(Map criteria,FlexType flexType,String name) throws Exception{
    ...
    */ 
      ...
      //if (LOGGER.isDebugEnabled())
        ...
        RFQResponse rfqResponseInput = (RFQResponse) LCSQuery.findObjectById(oid);
        RFQResponse rfqResponse = rfqResponseInput;
         try{
            rfqResponse = (RFQResponse) VersionHelper.latestIterationOf(rfqResponseInput);
        }catch(Exception e){
        }
        jSONObject.put("createdOn",FormatHelper.applyFormat(rfqResponse.getCreateTimestamp(),"DATE_TIME_STRING_FORMAT"));
        jSONObject.put("modifiedOn",FormatHelper.applyFormat(rfqResponse.getModifyTimestamp(),"DATE_TIME_STRING_FORMAT"));
        jSONObject.put("image",null);
        jSONObject.put("typeId",FormatHelper.getObjectId(rfqResponse.getFlexType()));
        jSONObject.put("ORid",FormatHelper.getObjectId(rfqResponse).toString());
        jSONObject.put("flexName",objectType);
        jSONObject.put("typeHierarchyName",rfqResponse.getFlexType().getFullNameDisplay(true));
        jSONObject.put("IDA2A2",FormatHelper.getNumericObjectIdFromObject(rfqResponse));
        jSONObject.put("BranchIdIterationInfo",FormatHelper.getNumericVersionIdFromObject(rfqResponse));
        ...
        Collection attributes = rfqResponse.getFlexType().getAllAttributes();
        Iterator it = attributes.iterator();
     	  while(it.hasNext()){
        	FlexTypeAttribute att = (FlexTypeAttribute) it.next();
        	String attKey = att.getAttKey();
        	try{
          		if(rfqResponse.getValue(attKey) == null){
           		 	jSONObject.put(attKey,"");
          		}
          		else{
           			jSONObject.put(attKey,rfqResponse.getValue(attKey).toString());
         		}
       		}catch(Exception e){
           	}
    	}
      DataConversionUtil datConUtil=new DataConversionUtil();
      return datConUtil.convertFlexTypesToJSONFormat(jSONObject,objectType,FormatHelper.getObjectId(rfqResponse.getFlexType()));	
    }  
}