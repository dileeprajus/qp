...
public class PaletteMaterialColorLinkModel extends GenericObjectService {

	//private static final Logger LOGGER = LogR.getLogger(PaletteMaterialColorLinkModel.class.getName());
	//private static final String CLASSNAME = PaletteMaterialColorLinkModel.class.getName();

	AppUtil util = new AppUtil();
	Gson gson = new Gson();

	/**
	 * This method is used to update the Color flex object that are passed by using
	 * OID as reference. Using this method we can update several records of a Color
	 * flextype at a time.
	 * 
	 ...
	 */
	public String updatePaletteMaterialColor(Map paletteMaterialColorScopeAttrs, String paletteMaterialOid)
			throws WTException {
		...
		String oid = "";
		//LOGGER.debug((Object) (CLASSNAME + "***** calling dataString creation ***** " ));
		String dataString = createPackagedStringfromRequest(paletteMaterialColorScopeAttrs, paletteMaterialOid);
		//LOGGER.debug((Object) (CLASSNAME + "***** dataString  ***** "+dataString));
		...
		//if (LOGGER.isDebugEnabled())
		{
			//LOGGER.debug((Object) (CLASSNAME + "***** Update PaletteColor dataString  ***** "+ dataString));
			//LOGGER.debug((Object) (CLASSNAME + "***** Update PaletteColor failedRows  ***** "+ failedRows));
			//LOGGER.debug((Object) (CLASSNAME + "***** Update PaletteColor results  ***** "+ results));
		}
	...
	/**
	 * This method is used to insert the Color flex object that are passed by using
	 * type as reference. Using this method we can insert several records of a Color
	 * flextype at a time.
	 * 
	...
	public JSONObject createPaletteMaterialColor(String type, JSONObject paletteMColorJsonObject) throws Exception {
		//if (LOGGER.isDebugEnabled())
		...
		try {
			...
			String mColorOid = (String) paletteMColorJsonObject.get("materialColorOid");
			String paletteOid = (String) paletteMColorJsonObject.get("paletteOid");
			//if (LOGGER.isDebugEnabled())
			//LOGGER.debug((Object) (CLASSNAME + "*****createPaletteMaterialColor  paletteOid +"+paletteOid+ " *****materialColorOid " + mColorOid));
			LCSMaterialColor materialColor = (LCSMaterialColor) LCSQuery.findObjectById(mColorOid);
			LCSPalette palette = (LCSPalette) LCSQuery.findObjectById(paletteOid);
			Collection<String> ids = MOAHelper.getMOACollection(mColorOid);

			paletteModel.load(paletteOid);
			//if (LOGGER.isDebugEnabled())
			//LOGGER.debug((Object) (CLASSNAME + "*****createPaletteMaterialColor Before adding material"));
            ArrayList<LCSMaterialColor> materialColors = new ArrayList();
            materialColors.add(materialColor);
			//Collection<String> ids = MOAHelper.getMOACollection(newIds);
            //LCSColorHelper.service.addMaterialColorsToPalette(ids, paletteModel.getBusinessObject());
            LCSPaletteLogic PALETTE_LOGIC = new LCSPaletteLogic();
            Collection<LCSPaletteMaterialColorLink> links = PALETTE_LOGIC.addMaterialColorsToPalette(materialColors, palette, null);
            LCSPaletteMaterialColorLink link = (LCSPaletteMaterialColorLink)links.iterator().next();
            String paletteMaterialColorOid = FormatHelper.getObjectId(link);
            //if (LOGGER.isDebugEnabled())
            //LOGGER.debug((Object) (CLASSNAME + "*****createPaletteMaterialColor paletteMaterialColorOid ** "+paletteMaterialColorOid));
            String paletteMCLinkid = updatePaletteMaterialColor(paletteMColorJsonObject, paletteMaterialColorOid);
            responseObject = util.getInsertResponse(paletteMCLinkid, type, responseObject);
	...
	/**
	 * This method is used either insert or update the Color flex object that are
	 * passed by using type as reference, oid and array of different colors data
	 * Using this method we can insert/update several records of a Color flextype at
	 * a time.
	 * 
	 * @param type           String
	 * @param oid            String
	 ...
	 * @return JSONArray It returns Color JSONArray object
	 */
	public JSONObject saveOrUpdate(String type, String oid, JSONObject searchJSONData, JSONObject payloadJson)
			throws Exception {
		//if (LOGGER.isDebugEnabled())
		...
		try {
			if (oid == null) {
				//LOGGER.debug((Object) (CLASSNAME + "*****saveOrUpdate before calling createPaletteMaterialColor  ** "));

				responseObject = createPaletteMaterialColor(type, payloadJson);

			} else {
				
				String palettemColorOid = updatePaletteMaterialColor(payloadJson, oid);
				
				
						//LOGGER.debug((Object) (CLASSNAME + "*****saveOrUpdate before calling updatePaletteMaterialColor with oid  ** "+palettemColorOid));
				responseObject = util.getUpdateResponse(palettemColorOid, type, responseObject);
	...
	public FlexTypeClassificationTreeLoader getFlexTypeHierarchy() throws Exception {
		return new FlexTypeClassificationTreeLoader("com.lcs.wc.color.LCSPaletteMaterialColorLink");
	}
			...
			if (configObject != null) {
				...
				if ((typeId != null) && !("".equalsIgnoreCase(typeId))) {
					...
					while (totalAttrsListItr.hasNext()) {
						...
						if (attribute.getAttScope().equals("PALETTE_MATERIALCOLOR_SCOPE")) {
	...
	public JSONObject getRecordByOid(String objectType, String oid) throws Exception {
		...
		LCSPaletteMaterialColorLink lcsPaletteMColorLinkInput = (LCSPaletteMaterialColorLink) LCSQuery
				.findObjectById(oid);
		LCSPaletteMaterialColorLink lcsPaletteMColorLink = lcsPaletteMColorLinkInput;
		try {
			lcsPaletteMColorLink = (LCSPaletteMaterialColorLink) VersionHelper
					.latestIterationOf(lcsPaletteMColorLinkInput);
		} catch (Exception e) {

		}
		jSONObject.put("createdOn",
				FormatHelper.applyFormat(lcsPaletteMColorLink.getCreateTimestamp(), "DATE_TIME_STRING_FORMAT"));
		jSONObject.put("modifiedOn",
				FormatHelper.applyFormat(lcsPaletteMColorLink.getModifyTimestamp(), "DATE_TIME_STRING_FORMAT"));
		//jSONObject.put("created By", "createdBy");
		jSONObject.put("typeId", FormatHelper.getObjectId(lcsPaletteMColorLink.getFlexType()));
		jSONObject.put("flexName", objectType);
		jSONObject.put("image", null);
		LCSMaterialMaster materialMaster = lcsPaletteMColorLink.getMaterialMaster();
		...
		palette = lcsPaletteMColorLink.getPalette();
		LCSMaterialColor materialColor = lcsPaletteMColorLink.getMaterialColor();
		String mcOid = FormatHelper.getObjectId(materialColor);
		...
		jSONObject.put("colorOid", FormatHelper.getObjectId(materialColor.getColor()));
		jSONObject.put("materialColorOid", mcOid);
		// jSONObject.put("oid",
		// FormatHelper.getVersionId(lcsPaletteMaterialLink).toString());
		jSONObject.put("oid", FormatHelper.getObjectId(lcsPaletteMColorLink).toString());
		jSONObject.put("ORid", FormatHelper.getObjectId(lcsPaletteMColorLink).toString());
		jSONObject.put("typeHierarchyName", lcsPaletteMColorLink.getFlexType().getFullNameDisplay(true));
		jSONObject.put("IDA2A2", FormatHelper.getNumericObjectIdFromObject(lcsPaletteMColorLink));
		String typeHierarchyName = (String) jSONObject.get("typeHierarchyName");
		 //required here
        String pattern = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'";
        SimpleDateFormat sdf = new SimpleDateFormat(pattern);
        String updated = sdf.format(lcsPaletteMColorLink.getModifyTimestamp());
		...
		Collection attributes = lcsPaletteMColorLink.getFlexType().getAllAttributes();
		Iterator it = attributes.iterator();
		while (it.hasNext()) {
			FlexTypeAttribute att = (FlexTypeAttribute) it.next();
			String attKey = att.getAttKey();
			try {
				if (lcsPaletteMColorLink.getValue(attKey) == null) {
					jSONObject.put(attKey, "");
				} else {
					jSONObject.put(attKey, lcsPaletteMColorLink.getValue(attKey).toString());
				}
			} catch (Exception e) {
			}
		}
		...
		return datConUtil.convertFlexTypesToJSONFormat(jSONObject, objectType,
				FormatHelper.getObjectId(lcsPaletteMColorLink.getFlexType()));
	}
	...
	public JSONObject getRecordByOid(String objectType, String oid, String association) throws Exception {
		//if (LOGGER.isDebugEnabled())
		{
			...
			//LOGGER.debug((Object) (CLASSNAME + "*****   getRecordByOid with asociation  ***** " + association));
		}
		...
		if (association.equalsIgnoreCase("Material")) {
			...
			jSONObject.put("Color", findPalette(oid));
		}
		return jSONObject;
	}
	 ...
	 * @return JSONArray it returns the palette associated Color records in the form
	 *         of array
	 */
	public JSONArray findMaterial(String paletteMaterialOid) {
		JSONArray materialObjectArray = new JSONArray();
		//if (LOGGER.isDebugEnabled())
			//LOGGER.debug((Object) (CLASSNAME + "***** Find colors associated with palletColorLinkid id  ***** "
				//	+ paletteMaterialOid));
		MaterialModel materialModel = new MaterialModel();
		JSONObject materialObject = new JSONObject();
		try {
			LCSPaletteMaterialColorLink lcsPaletteMColorLink = (LCSPaletteMaterialColorLink) LCSQuery
					.findObjectById(paletteMaterialOid);
			LCSMaterialMaster materialMaster = lcsPaletteMColorLink.getMaterialMaster();
			LCSMaterial material = (LCSMaterial) VersionHelper.latestIterationOf(materialMaster);
			String materialOid = FormatHelper.getObjectId(material);
	...
	public JSONArray findPalette(String paletteMaterialOid) {
		...
		//if (LOGGER.isDebugEnabled())
			//LOGGER.debug((Object) (CLASSNAME + "***** Find palette associated with paletteMaterialOid id  ***** "
				//	+ paletteMaterialOid));
	...
	public JSONArray findColor(String paletteColorOid) {
		...
		//if (LOGGER.isDebugEnabled())
			//LOGGER.debug((Object) (CLASSNAME + "***** findColor with paletteColorOid id  ***** "+ paletteColorOid));
	
		ColorModel colorModel = new ColorModel();
		JSONObject colorObject = new JSONObject();
		try {
			LCSPaletteMaterialColorLink lcsPaletteMColorLink = (LCSPaletteMaterialColorLink) LCSQuery
					.findObjectById(paletteColorOid);
			LCSMaterialColor mColor = lcsPaletteMColorLink.getMaterialColor();
			LCSColor color = mColor.getColor();
	...
	public static String createPackagedStringfromRequest(Map map, String oid) {
		...
		values.append("ID" + "|&^&|" + "LCSPALETTEMATERIALCOLORLINK" + "$" + id + "|-()-|");
