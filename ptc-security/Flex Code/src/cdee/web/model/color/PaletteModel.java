...
public class PaletteModel extends GenericObjectService {
    ...
    public JSONObject createPalette(String type, JSONObject paletteDataList) {
        //if (LOGGER.isDebugEnabled())
           //LOGGER.debug((Object) (CLASSNAME + "***** Create initialized type ***** "+ type));
        JSONObject responseObject = new JSONObject();
        LCSPaletteClientModel paletteModel = new LCSPaletteClientModel();
        try {
            DataConversionUtil datConUtil=new DataConversionUtil();
        ...
        try {
            if (oid == null) {
                ...
                if (!(list.get(2).toString()).equalsIgnoreCase("no record")) {
                    ...
                    //if (LOGGER.isDebugEnabled())
                        //LOGGER.debug((Object) (CLASSNAME + "***** saveOrUpdate before createPalette ")); 
                    
                    if(payloadJson.containsKey("parentPaletteOid"))
                    {
                        
                        String parentPaletteOid = (String) payloadJson.get("parentPaletteOid");
                        //if (LOGGER.isDebugEnabled())
                            //LOGGER.debug((Object) (CLASSNAME + "***** saveOrUpdate before createSubPalette with parentPaletteOid "+parentPaletteOid)); 
                        responseObject = createSubPalette(type,payloadJson, parentPaletteOid);
                    }
                    else
     ...
     */
    public JSONObject createSubPalette(String type,JSONObject paletteJson, String parentPaletteOid) {
        //if (LOGGER.isDebugEnabled())
