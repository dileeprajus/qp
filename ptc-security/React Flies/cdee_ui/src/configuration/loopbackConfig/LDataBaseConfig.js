...
class LDataBaseConfig {
    ...
    createConfigCB(response) {
        if (response.data && response.data) {
            ...
            if (response.data['result'].tenantId !== undefined)
                response.data['result'].TenantId = response.data['result'].tenantId;
        }
...
export default LDataBaseConfig;
