...
function constructObjAssignValue(parent_obj, parent_key, output_obj, output_delim_arr, input_value, curr_mapped_conf, meta_details) {
    ...
    // If output obj type is an array but output first key is not a number then default number to zero
    if(Array.isArray(output_obj)){
        if(isNaN(output_delim_arr[0])){
            // If object is an array iterate number of elements in the array and join the output
            var arr_length = output_obj.length;
            for(var arr_ind=0; arr_ind < arr_length; arr_ind++){
                if(isNaN(output_delim_arr[0])){
                    output_delim_arr.unshift(arr_ind);
                }else{
                    output_delim_arr[0] = arr_ind;
                }
                constructObjAssignValue(parent_obj, parent_key, output_obj, output_delim_arr, input_value, curr_mapped_conf, meta_details)
            }
    ...
    if (input_delimiter_arr_cpy.length > 1) {
        ...
        var curr_type_is_array = false;
        // Remove brackets from key string in output
        if (cur_key[0] === '[' && cur_key[cur_key.length - 1] === ']') {
            cur_key = cur_key.slice(1, -1);
            curr_type_is_array = true;
        }

        // Deal objects for the first time initiate of a subset with {} or []
        if (output_obj[cur_key] === undefined || output_obj[cur_key] === null) {
            ...
            if(!isNaN(cur_key)){
                ...
                if(curr_type_is_array === true){
                    output_obj[cur_key] = [];
                    var array_index = 0;
                    if(input_delimiter_arr_cpy[0] === array_index){
                        // for now do nothing
                    }else{
                        input_delimiter_arr_cpy.unshift(array_index);
                    }
                }else{
                    output_obj[cur_key] = {};
                }
...
function attrKeyArrayIter(master_input_obj, input_delimi_arr, output_data, ouput_delim_arr, is_it_first_time, curr_mapped_conf, meta_details) {
    ...
    if (master_input_obj) {
        ...
        if (current_input_key[0] === '[' && current_input_key[current_input_key.length - 1] === ']') {
            ...
            //uncomment below 3 lines in Thingworx
            // current_input_key = current_input_key.slice(1, -1);
            // current_input_key =  current_input_key.replace(/ +/g, ""); // remove the space between words in string
            // var tmp_arr_obj = master_input_obj[current_input_key];
            if(tmp_arr_obj!==undefined){
            for (var tmp_i = 0; tmp_i < tmp_arr_obj.length; tmp_i++) {
                ...
                var is_there_an_array_in_output_parents = false;
                ...
                for(var tmp_o_i = ouput_delim_arr_copy.length-1; tmp_o_i>-1; tmp_o_i--){
                    ...
                    if (curr_o_key[0] === '[' && curr_o_key[curr_o_key.length - 1] === ']') {
                        is_there_an_array_in_output_parents = true
                        // if nearparent key is array insert index as delimiter
                        ouput_delim_arr_copy.splice(tmp_o_i+1, 0, tmp_i);
                    }

                    if(tmp_o_i === 0){
                        if(is_there_an_array_in_output_parents === false){
                            ouput_delim_arr_copy.splice(0, 0, tmp_i);
                            // if(tmp_i === 0){
...
function sortedMappingIter(master_input_obj, sorted_mapped_iter, output_data, meta_details) {
    ...
    if(current_mapped_obj!==undefined){
...
function getConvertedOutput(mappingConfigJson, coming_from, input_data) {
    ...
    for (var i = 0; i < mapping_schema.length; i++) {
        ...
        var input_subset_schema = input_schema[i_typeName];
        ...
        var tmp_index = 0;
        var tmp_obj = [];
        // This for loop is optional to sort according to the length of hierarchy attr
        for (var j = 0; j < sorted_attr_map.length; j++) {
            if (i_key_arr.length < sorted_attr_map[j]['i_key_arr'].length) {
                tmp_index = j
                break;
            } else {
        ...
        sorted_attr_map.splice(tmp_index,0, { i_key_arr: i_key_arr, o_key_arr: o_key_arr, curr_obj: mapping_schema[i] });
    }
    ...
    var final_tmp_out = sortedMappingIter(input_data, sorted_attr_map.slice(), output_data, meta_details);
