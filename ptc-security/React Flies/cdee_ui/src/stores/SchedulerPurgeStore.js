...
class SchedulerPurgeStore {
  constructor(name) {
    ...
    this.store_name = 'SchedulerPurgeStore'; //This is the filename
  }
  ...
  @observable purgeScheduleTypes = ['Days', 'Weeks', 'Months'];
  @observable dateDurationTypes =['Select','Last 1 Week', 'Last 2 Weeks', 'Last 1 Month', 'Last 2 Months', 'Last 1 Year'];
  @observable currentScheduleType = 'Days';
  @observable currentDateDurationType = 'Select';
...
//Inputs: {input:{config_name:name of the thing ex: WH/Static/Rest etc}}
  ...
  getPurgeDetailsConfig() {
    
    var schedulerThis = this;
    ieGlobalVariable.loaderStore.turnon();
    const apiInput = {
      midpoint: 'ThingSchedulerToPurgeEntries',
      endpoint: 'GetPurgeDetailsConfig',
      methodType: 'POST',
      payload: {},
      queryparam: null,
      headers: null
    ...
    backendServer.api_call(apiInput)
      .then(function (response) {
        ...
        schedulerThis.currentScheduleType = response.data.result.schedulerType ? response.data.result.schedulerType :'Days';
        schedulerThis.currentDateDurationType = response.data.result.DateDuration ? response.data.result.DateDuration :'Select';
        ieGlobalVariable.loaderStore.turnoff();
        schedulerThis.getTotalConfigs;
        if (schedulerThis.async_callback) {
          schedulerThis.async_callback(schedulerThis, response);
 ...
 saveSchedulerCron(configName) {
   var searchQuery = {
     ...
     dateDuration:this.currentDateDurationType
   }
   var schedulerThis = this;
   ieGlobalVariable.loaderStore.turnon();
   const apiInput = {
...
export default SchedulerPurgeStore;
