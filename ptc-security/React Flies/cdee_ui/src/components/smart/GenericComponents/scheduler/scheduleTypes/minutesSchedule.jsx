...
class MinutesSchedule extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentWillReceiveProps() {
    if(this.props.scheduler_store.newCronString) {
      ...
      if(tempArr[1]) {
        ...
        tempArr[1] = (tempArr[1][1] !== undefined && tempArr[1][1] !== ''&& tempArr[1][1]!=='*') ? tempArr[1][1] : '0'
      }
  ...
  selectItem(event) {
    ...
    var cronStr = this.scheduler_store.retrunCronExpression('Minutes',
      {
  ...
  render() {
    ...
    var tableBody = [];
    tableBody.push(
      <tr key={'minutesScheduleTableRow'}>
        <td> Every </td>
        <td>
...
MinutesSchedule.propTypes = {
  store: React.PropTypes.object
};

export default MinutesSchedule;
