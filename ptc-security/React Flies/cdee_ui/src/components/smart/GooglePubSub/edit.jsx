...
@observer
class GooglePubSubEdit extends React.Component {
  constructor(props) {
    super(props);
    // This is not creating new datasource
    this.client_store = new GooglePubSubClientStore(this.props.match.params.name);
  ...
  onChange(event) {
    var bCObj = this.client_store.googlePubSubEditFileInfo;
      var file = document.getElementById('fileUploadForGooglePubSubEdit').files[0];
      if(event.target.name === 'fileContent') {
          if (file) {
            var fNameArry = file.name.split('.');
              ...
              if(fName.toUpperCase() === bCObj.fileType) {
                  ...
                  document.getElementById('fileUploadForGooglePubSubEdit').value = '';
              }}
      }else {
        if(event.target.name === 'fileType') {
          ...
          bCObj[event.target.name] = event.target.value;
          this.state.topic_name = event.target.value;
        }else if(event.target.name === 'subscriptionName'){
          bCObj[event.target.name] = event.target.value;
        }
        else{}
     }
      this.client_store.setvalue('googlePubSubEditFileInfo', bCObj);
      if(event.target.name === 'topicName'){
      this.client_store.subscriptionList = this.client_store.GetListOfSubscriptions(this.state.topic_name);
      }
  }
  ...
  handleSubmit(event) {
    var obj = {};
    obj.filePath = this.client_store.googlePubSubEditFileInfo.filePath;
    obj.fileType = this.client_store.googlePubSubEditFileInfo.fileType;
    obj.fileData = this.client_store.googlePubSubEditFileInfo.fileData;
    obj.topicName = this.client_store.googlePubSubEditFileInfo.topicName;
    obj.subscriptionName = this.client_store.googlePubSubEditFileInfo.subscriptionName;
    if ((this.client_store.googlePubSubEditFileInfo.fileType === 'XML') || (this.client_store.googlePubSubEditFileInfo.fileType === 'CSV')) {
     if(this.client_store.googlePubSubEditFileInfo.fileData === '/"error/":/"failed/"') {
       obj.fileData = '';
     }
    }else {
      if(this.client_store.googlePubSubEditFileInfo.fileData) { 
        if(this.client_store.googlePubSubEditFileInfo.fileData === '/"error/":/"failed/"') {
          obj.fileData = '';
        }
    ...
    obj.csvDelimeter = this.client_store.googlePubSubEditFileInfo.csvDelimeter;
    this.client_store.testGooglePubSubData(obj);
    event.preventDefault();
  }

  deleteConfig() {
    this.setState({ service_name: 'Delete', custom_path: '/GooglePubSub' });
  ...
  saveGooglePubSubConfig() {
    ...
    tempConfig['googlePubSubEditFileInfo'] = this.client_store.googlePubSubEditFileInfo;
    ...
    this.client_store.SetPropValues({
      googlePubSubEditFileInfo: this.client_store.googlePubSubEditFileInfo,
  ...
  render() {
    ...
    fileType.map(type => {
      fileTypeList.push(
        <option key={type} selected={this.client_store.googlePubSubEditFileInfo.fileType === type} value={type} disabled={(type=== 'XLS' || type ==='STRING' || type === 'XML' || type === 'CSV')?true:false}>{type}</option>
      );
    });
    }
    ...
    fileType.map(type => {
      fileTypeList.push(
        <option key={type} selected={this.client_store.googlePubSubEditFileInfo.fileType === type} value={type} disabled={(type=== 'XLS' || type ==='STRING' || type ==='CSV')?true:false}>{type}</option>
      );
    });
    }
    var topicName = this.client_store.topicsList;
    var topicNameList = [];
    if(this.props.generic_master_store.groupType === 'target')
    {
      topicName.map(type => {
        topicNameList.push(
          <option key={type} selected={this.client_store.googlePubSubEditFileInfo.topicName === type} value={type} >{type}</option>
        );
      });
    }
    else if(this.props.generic_master_store.groupType === 'source')
    {
      topicName.map(type => {
        topicNameList.push(
          <option key={type} selected={this.client_store.googlePubSubEditFileInfo.topicName === type} value={type} >{type}</option>
    ...
    var subscriptionName = this.client_store.subscriptionList;
    var subscriptionNameList = [];
    if(this.props.generic_master_store.groupType === 'source' && subscriptionName.length !== 0)
    {
      subscriptionName.map(type => {
        subscriptionNameList.push(
          <option key={type} selected={this.client_store.googlePubSubEditFileInfo.subscriptionName === type} value={type} >{type}</option>
        );
      });
    }
    else if(this.props.generic_master_store.groupType === 'source' && subscriptionName.length === 0)
    {
    ...
    fileEncodeType.map(type => {
      fileEncodingTypeList.push(
        <option key={type} selected={this.client_store.googlePubSubEditFileInfo.fileEncodeType === type} value={type}>{type}</option>
      );
    });
    ...
    var data = '';
      if (this.client_store.googlePubSubEditFileInfo.fileType === 'JSON') {
        data = (<CodeMirror id="FileData" name="data" onChange={this.updateTargetData.bind(this)}
                            value={JSON.stringify((this.client_store.googlePubSubEditFileInfo.fileData === '') ? {}:this.client_store.googlePubSubEditFileInfo.fileData, null, 2)}
                            options={codeOptions}/>)
      } else {
        data = (<CodeMirror id="FileData" name="data" onChange={this.updateTargetData.bind(this)}
                            value={(this.client_store.googlePubSubEditFileInfo.fileData === '') ? '\"\"':this.client_store.googlePubSubEditFileInfo.fileData} options={codeOptions}/>)
      }
    ...
    return (
      <div>
        ...
        <Col xs={12}>
          <Row>
            <Navigator history={this.props.history} action={'Edit'} type={'/GooglePubSub' + ''} tempStore={this.client_store} delete={this.deleteConfig.bind(this)} save={this.saveGooglePubSubConfig.bind(this)} source={'Thing'} />
                ...
                <Grid>
                  <Row>
                    <Col xs={10} xsOffset={1}>
                      <h3>GooglePubSub Configuration</h3>
                      <Thumbnail>
                        <form onSubmit={this.handleSubmit}>
                          <FormGroup>
                            <ControlLabel>Topic Name</ControlLabel>
                            <FormControl componentClass="select" placeholder="Topic Name" name='topicName' onChange={this.onChange.bind(this)}>
                              <option key='Default'  value="">Select Topic Name</option>
                              {topicNameList}
                            </FormControl>
                          </FormGroup>
                          <FormGroup hidden={this.props.generic_master_store.groupType === 'target'}>
                            <ControlLabel>Subscription Name</ControlLabel>
                            <FormControl componentClass="select" placeholder="Subscription Name" name='subscriptionName' onChange={this.onChange.bind(this)}>
                              <option key='Default'  value="">Select Subscription Name</option>
                              {subscriptionNameList}
                            </FormControl>
                          </FormGroup>
                          <FormGroup>
                          ...
                          <Row>
                            <Col xs={6} sm={6} md={6} lg={6}>
                              <FormGroup hidden={this.client_store.googlePubSubEditFileInfo.fileType !== 'CSV'}>
                                <FormControl type="text" placeholder="delimeter" name='csvDelimeter' value={this.client_store.googlePubSubEditFileInfo.csvDelimeter} onChange={this.onChange.bind(this)}/>
                              </FormGroup>
                            </Col>
                            <Col xs={6} sm={6} md={6} lg={6}>
                              <FormGroup hidden={this.client_store.googlePubSubEditFileInfo.fileType === '' || this.client_store.currentGroupType === 'source'}>
                                <input type="file" id="fileUploadForGooglePubSubEdit" name="fileContent"
                          ...
                          <Row>
                          <Col xs={12} id="googlePubSubJsonPretty" style={{ backgroundColor: '#282A36' }}>
                            <Col xs={4} sm={4} md={4} lg={4}>
                              ...
                              <OverlayTrigger placement="top" overlay={test_data_tooltip} hidden={this.client_store.currentGroupType === 'target'}>
                                <Button
                                  ...
                                  disabled={this.client_store.googlePubSubEditFileInfo.fileType === ''}
                                >
                            ...
                            <Col xs={4} sm={4} md={4} lg={4}>
                              ...
                              <FormGroup hidden={this.client_store.googlePubSubEditFileInfo.fileType === ''}>
                                {data}
                              </FormGroup>
                            </Col>
                            <Col xs={4} sm={4} md={4} lg={4}>
...
GooglePubSubEdit.propTypes = {
  store: React.PropTypes.object
};

export default GooglePubSubEdit;
