...
class GooglePubSubNew extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentWillMount() {
    this.props.GooglePubSubClientStore.setvalue('currentGroupName', this.props.generic_master_store.currentGroupName);
    var thingExists = this.props.GooglePubSubClientStore.config_names.indexOf((this.props.GooglePubSubClientStore.new_config_name).toUpperCase()) === -1;
    if (!thingExists) {
  ...
  onChange(event) {
    ...
    this.props.GooglePubSubClientStore.setvalue(event.target.name, tempEventText);
    this.setState({ show_specialchar_err_msg: false });
    this.v_store.state.custom_store = this.props.GooglePubSubClientStore;
    this.v_store.ValidateThingCreate('onchange');
    if (event.target.name === 'new_config_name') {
      var config_names = this.props.GooglePubSubClientStore.currentGroupName+'-'+tempEventText;
      if (this.props.GooglePubSubClientStore.config_names.indexOf(config_names.toUpperCase()) !== -1) {
        this.setState({ show_err_msg: true });
  ...
  handleSubmit() {
    this.v_store.state.custom_store = this.props.GooglePubSubClientStore;
    const name = this.props.GooglePubSubClientStore.new_config_name.length;
    const description = this.props.GooglePubSubClientStore.new_config_description.length;
    if ((name > 1) && (description > 2)) {
      this.props.GooglePubSubClientStore.CreateConfig(this.props.generic_master_store, this.redirectToEdit.bind(this));
  ...
  redirectToEdit() {
    this.props.history.push('/'+this.props.url.split('/')[1] + '/Edit/' + this.props.GooglePubSubClientStore.name);
  }

  render() {
    const tooltip = (
      <Tooltip id="tooltip">
        <strong>Create New GooglePubSubConfig.!</strong><br />Click this...
      </Tooltip>
    );
    return (
      <div>
        <form onSubmit={this.handleSubmit}>
          ...
          >
            <FormControl
              ...
              value={this.props.GooglePubSubClientStore.new_config_name}
          ...
          <FormGroup validationState={this.v_store.state.custom_description}>
            <FormControl
              ...
              value={this.props.GooglePubSubClientStore.new_config_description}
              onChange={this.onChange.bind(this)}
            />
            <FormControl.Feedback />
        ...
        <OverlayTrigger placement="top" overlay={tooltip}>
          <Button
            ...
            disabled={this.state.show_err_msg || this.props.GooglePubSubClientStore.new_config_name.indexOf(' ') > -1 || this.props.GooglePubSubClientStore.new_config_name.length < 3 || this.props.GooglePubSubClientStore.new_config_description.length < 3}
          >Submit</Button>
        </OverlayTrigger>
        <ModalInstance custom_store={this.props.GooglePubSubClientStore} service_name="CreateConfig" />
      </div>
    );
  }
}

GooglePubSubNew.propTypes = {
  store: React.PropTypes.object
};

export default GooglePubSubNew;
