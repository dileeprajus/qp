...
import TestApisScreen from '../RestClient/components/test_apis_screen';
import { RIEInput, RIESelect } from 'riek';
import _ from 'lodash';

@inject('breadcrumb_store', 'modal_store', 'validation_store', 'generic_master_store')
@observer
class DataBaseConfigEdit extends React.Component {
  constructor(props) {
    super(props);
    // This is not creating new datasource
    this.client_store = new DataBaseStore(this.props.match.params.name);
  ...
  onChange(event) {
      ...
      if(event.target.name === 'query') {
          temp_sourceConfig.query = event.target.value;
  ...
  handleSubmit(event) {
    this.setState({ service_name: 'SetHostProperties', custom_path: '/DataBase/Show/' });
  ...
  deleteConfig() {
    this.setState({ service_name: 'Delete', custom_path: '/DataBase' });
  ...
  saveDataBaseSchema() {
    ...
    var DataBaseConfig = this.client_store.configJson;
    DataBaseConfig['PrimaryKey'] = pk;
    this.client_store.setvalue('configJson', DataBaseConfig);
    this.client_store.SetPropValues({ outputSchema: this.client_store.outputSchema, inputSchema: this.client_store.inputSchema, ConfigJson: this.client_store.configJson});
  ...
  fetchSchema() {
      this.client_store.fetchSchemaFromRawData();
  }
  ...
  { id: 'time', text: 'time' }]
    ...
    return (
      <div>
        <Col xs={12}>
          <Row>
            <Navigator history={this.props.history} action={'Edit'} type={'/DataBase'} tempStore={this.client_store} delete={this.deleteConfig.bind(this)} save={this.saveDataBaseSchema.bind(this)} source={'Thing'} />
                    ...
                    <Col xs={12}>
                      <h4>DataBase Query</h4>
                        <form>
                          ...
                          <Col xs={6} xsOffset={2}>
                            ...
                            <FormGroup>
                              <FormControl componentClass="textarea" style={{ resize: 'none' }} placeholder="Enter Query" name="query" value={this.client_store.configJson.query}
                      ...
                      <Col xs={12} id="socket_test_schema_json-pretty" style={{ backgroundColor: '#282A36' }}>
                        ...
                        <Col xs={4} sm={4} md={4} lg={4} >
                          ...
                          <TestApisScreen
                            rest_client_store={this.client_store}
                            test_data={this.client_store['test_response']}
                            data_format='JSON'
                          />
                        </Col>
                        <Col xs={4} sm={4} md={4} lg={4} >
...
DataBaseConfigEdit.propTypes = {
  store: React.PropTypes.object
};

export default DataBaseConfigEdit;
