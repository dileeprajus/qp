...
@observer
class SocketNew extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentWillMount() {
    this.props.socketStore.setvalue('currentGroupName', this.props.generic_master_store.currentGroupName);
    var configExists = this.props.socketStore.config_names.indexOf((this.props.socketStore.new_config_name).toUpperCase()) === -1;
    if (!configExists) {
      this.setState({ show_err_msg: true });
    }
    this.v_store.InitialState();
    this.props.socketStore.getSocketConfigs;
  }
  onChange(event) {
    ...
    this.props.socketStore.setvalue(event.target.name, tempEventText);
    this.setState({ show_specialchar_err_msg: false });
    this.v_store.state.custom_store = this.props.socketStore;
    this.v_store.ValidateThingCreate('onchange');
    if (event.target.name === 'new_config_name') {
      const config_names = this.props.socketStore.currentGroupName + '-' + tempEventText;
      if (this.props.socketStore.config_names.indexOf(config_names.toUpperCase()) !== -1) {
        this.setState({ show_err_msg: true });
  ...
  handleSubmit() {
    this.v_store.state.custom_store = this.props.socketStore;
    const name = this.props.socketStore.new_config_name.length;
    const description = this.props.socketStore.new_config_description.length;
    if ((name > 1) && (description > 2)) {
      this.props.socketStore.CreateConfig(this.props.generic_master_store, this.props.enableModalTabs.bind(this));
    } else {
  ...
  render() {
    const tooltip = (
      <Tooltip id="tooltip"><strong>Create New SocketConfig...!</strong><br />Click this...</Tooltip>
    );
    return (
      <div>
        <form onSubmit={this.handleSubmit}>
          ...
          >
            <FormControl
              ...
              value={this.props.socketStore.new_config_name}
          ...
          <FormGroup validationState={this.v_store.state.custom_description}>
            <FormControl
              ...
              value={this.props.socketStore.new_config_description} onChange={this.onChange.bind(this)}
            />
            <FormControl.Feedback />
        ...
        <OverlayTrigger placement="top" overlay={tooltip}>
          <Button
            ...
            disabled={this.state.show_err_msg || this.props.socketStore.new_config_name.indexOf(' ') > -1 || this.props.socketStore.new_config_name.length < 3 || this.props.socketStore.new_config_description.length < 3}
          >Submit</Button>
        </OverlayTrigger>
        <ModalInstance custom_store={this.props.socketStore} service_name="CreateConfig" />
      </div>
    );
  }
}
